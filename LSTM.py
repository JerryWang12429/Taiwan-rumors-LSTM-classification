...
from keras.layers import Dense, Input, GlobalAveragePooling1D, GlobalMaxPooling1D, concatenate, Activation
from keras.layers import MaxPooling1D, Conv2D, Reshape, MaxPooling2D, ConvLSTM2D, TimeDistributed
from keras.layers.core import Flatten
from keras.models import Model, Sequential
from keras.layers import Embedding, LSTM, Dropout, Bidirectional, Conv1D, GRU
...
def main():
    ...
    # dimension = (sequence_length-filter_size-1)*(EMBEDDING_DIM-filter_size-1)*filter_num
    ...
    maxpool_0 = MaxPooling2D((sequence_length/5, 1),
                             strides=(1, 1))(conv_0)

    maxpool_1 = MaxPooling2D((sequence_length/5, 1),
                             strides=(1, 1))(conv_1)
    maxpool_2 = MaxPooling2D((sequence_length/5, 1),
                             strides=(1, 1))(conv_2)

    merged_tensor = concatenate([maxpool_0, maxpool_1, maxpool_2], axis=1)
    dropout = Dropout(drop)(merged_tensor)
    flatten = Flatten()(dropout)
    reshape2 = Reshape((1, merged_tensor.shape[1].value * merged_tensor.shape[3].value))(flatten)
    lstm = LSTM(100)(reshape2)

    output = Dense(units=2,
                   activation='softmax',
                   kernel_regularizer=regularizers.l2(penalty))(lstm)
